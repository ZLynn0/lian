	...
	$('#btnsave').on('click',function(){
		...
		if(isValid){
			$.ajax({
				type:'post',
				data:$('#f1').serialize(),
				url:'/project1/publish.do?type=add',
		...
		toolbar:[{
							...
							handler:function(){
								...
								if(isValid){
									$.ajax({
										type:'post',
										data:$('#f1').serialize(),
										url:'/project1/publish.do?type=edit&pid='+row.pid,
										success:function(data){
											if(data=="1"){
			...
			handler: function() {
				...
				if(row==null){
					...
					$.messager.confirm("删除确认","确定删除"+row.pname+"?",function(r){
						if(r){
							$.ajax({
								type:'post',
								url:'/project1/publish.do',
